    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">添加商品名称</h4>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>

            <!-- 模态框主体 -->
            <div class="modal-body">
                <form method="post" action="${app}/categorys/list" enctype="multipart/form-data"  class="form-horizontal" role="form">
                    <%--input type="hidden" name="_method" value="POST" /--%>
                    <div class="form-group">
                        <lable>cgname:</lable>
                        <input type="text" class="form-control"  name="cgname"
                               placeholder="添加商品名称"/>
                    </div>
                    <div class="form-group">
                        <label>cdes:</label>
                        <textarea class="form-control" name="gdes" placeholder="添加商品描述"></textarea>
                    </div>
                        <div class="form-group">
                            <label>cavatar:</label>
                            <img data-my="disAvatar" src="" style="width: 100px;height: 100px;" />
                            <input style="display: none;" type="file" class="form-control" data-my="inputAvatar" name="file" placeholder="cavatar">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="post" enctype="multipart/form-data" class="form-horizontal" role="form">
                    ...
                    <div class="form-group">
                        <label>cgid:</label>
                        <input readonly="readonly" type="text" class="form-control" name="cgid"/>
                    </div>
                    <div class="form-group">
                        <label>cgname:</label><input type="text" class="form-control" name="cgname" />
                    </div>

                    <div class="form-group">
                        <label>cdes:</label><textarea class="form-control"  name="cdes" ></textarea>
                    </div>
                    <div class="form-group">
...
<form id="searchForm" method="get" action="${app}/categorys/list">

    <input type="text" placeholder="cgname" name="cgname" value=""/>
...
<script>
    ...
    function updateForm(ele) {
        ...
        $.ajax({
            ...
            success: function (result) {
                //回填数据
                $('#updateModal [name="cgid"]').val(result.dataZone.obj.cgid);
                $('#updateModal [name="cgname"]').val(result.dataZone.obj.cgname);
                $('#updateModal [name="cdes"]').val(result.dataZone.obj.cdes);
    ...
    function deleteMuliRecord() {
        ...
        var cgids = "";//需要传递给服务器的uid列表
        var cgnames = "";//需要显式给操作者看的提示信息列表
        $("[name=choiceList]:checkbox").each(function () {
            if (this.checked) {
                cgids += $(this).parents("tr").find("td:eq(0)").text() + "-";//通过 - 连接
                cgnames += $(this).parents("tr").find("td:eq(1)").text() + ",";//通过 , 连接
            }
        });
        cgids = cgids.substr(0, cgids.length - 1);//去掉最后的一个 -
        cgnames = cgnames.substr(0, cgnames.length - 1);//去掉最后的一个 ,
        //询问用户操作
        if (confirm("是否删除" + cgnames + "的记录")) {
            //向服务器发送请求,我们已经使用过get和post方法,这次使用最底层的ajax方法
            $.ajax({
                type: "DELETE",
                url: "${app}/categorys/opt/" + cgids,
                success: function (result) {
                    gotoPage(currentPage);
                    alertTips(result.message,"alert-success");
                },
    ...
    function gotoPage(page, pageSize) {
        ...
        $.ajax({
            type: "GET",
            url: "${app}/categorys/list?pageNum=" + page1 + "&pageSize=" + pageSize1,
            dataType: "json",
            data: $("#searchForm").serialize(),
            success: function (result) {
    ...
    function parseDataAndShow(result) {
        ...
        $.each(lists, function (index, item) {
            ...
            var uTr = $("<tr></tr>");
            //构建多个单元格
            var checkboxTh = $('<th><input type="checkbox" name="choiceList" value="${item.cgid}"/></th>');
            var countTh = $('<th></th>').text(index + 1);
            var td1 = $('<td></td>').text(item.cgid);
            var td2 = $('<td></td>').text(item.cgname);
            var td3 = $('<td></td>').text(item.cdes);
            var td4 = $('<td></td>').text(item.cavatar);
            var addTimeTd = $('<td></td>').text(new Date(item.addTime).Format("yyyy-MM-dd HH:mm:ss"));
            var upBtnTd = $('<td></td>').html('<a class="upBtn btn btn-info btn-sm" href="${app}/categorys/opt/' + item.cgid + '">修改</a>');
            var delBtnTd = $('<td></td>').html('<a class="delBtn btn btn-danger btn-sm" href="${app}/categorys/opt/' + item.cgid + '">删除</a>');
            //将单元格追加到行中
            uTr.append(checkboxTh).append(countTh).append(td1)
                .append(td2).append(td3).append(td4)
                .append(addTimeTd)
