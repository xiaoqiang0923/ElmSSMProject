    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="${app}/goods/opt" enctype="multipart/form-data" class="form-horizontal" role="form">
                    <div class="form-group">
                        <label>gname:</label><input type="text" class="form-control"  name="gname" placeholder="商品名称"/>
                    </div>
                        <div class="form-group">
                            <label>gimg:</label>
                            <img data-my="disAvatar" src="" style="width: 100px;height: 100px;"/>
                            <input style="display: none;" type="file" class="form-control" data-my="inputAvatar" name="file" placeholder="商品图片">
                        </div>
                    <div class="form-group">
                        <label>gintroduction:</label><textarea class="form-control" name="gintroduction" placeholder="商品介绍"></textarea>
                    </div>
                    <div class="form-group">
                        <label>gprice:</label><input type="text" class="form-control"  name="gprice" placeholder="商品价格"/>
                    </div>
                    <div class="form-group">
                        <label>gbid:</label><input list="glist" type="text" class="form-control"  name="gbid" placeholder="所属商户"/>
                        <datalist id="glist">
                        </datalist>
                    </div>
                    <div class="form-group">
                        <label>gcgid:</label><input list="gclist" type="text" class="form-control"  name="gcgid" placeholder="所属类别"/>
                        <datalist id="gclist">
                        </datalist>
                    </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="${app}/goods/opt"  enctype="multipart/form-data" class="form-horizontal" role="form">
                    <div class="form-group">
                        <label>gid:</label><input readonly="readonly" type="text" class="form-control"  name="gid"  placeholder="商品id"/>
                    </div>
                    <div class="form-group">
                        <label>gname:</label><input type="text" class="form-control"  name="gname"  placeholder="修改商品名称"/>
                    </div>
                    <div class="form-group">
                        <label>gimg:</label>
                        <img data-my="disAvatar" src="" style="width: 100px;height: 100px;" />
                        <input style="display: none;" type="file" class="form-control" data-my="inputAvatar" name="file" placeholder="修改商品图片">
                    </div>
                    <div class="form-group">
                        <label>gintroduction:</label><textarea class="form-control" name="gintroduction" placeholder="修改商品描述"></textarea>
                    </div>
                    <div class="form-group">
                        <label>gprice:</label><input type="text" class="form-control"  name="gprice" placeholder="修改商品价格"/>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label>gbid:</label><input list="uglist" type="text" class="form-control"  name="gbid" placeholder="修改所属商户"/>
                        <datalist id="uglist">
                        </datalist>
                    </div>
                    <div class="form-group">
                        <label>gcgid:</label><input list="ugclist" type="text" class="form-control"  name="gcgid" placeholder="修改所属类别"/>
                        <datalist id="ugclist">
                        </datalist>
                    </div>

                    <div class="form-group">
                        <label>addTime:</label><input type="datetime" class="form-control"  name="addTime" placeholder="修改时间"/>
                    </div>
...
<form id="searchForm" method="get" action="${app}/goods/list">
...
<div>
    ...
    <input class="btn btn-danger" type="button" action="${app}/goods/opt" id="deletesBtn" value="删除所选"/>
</div>
...
// 页面专用的代码从这里开始
    ...
    $("#openAddModalBtn").click(function (){
        getAndFill("${app}/business/listJSON",$('#addModal[id="glist"]'),"bid","bname");
        getAndFill("${app}/categorys/listJSON",$('#addModal[id="gclist"]'), "cgid", "cgname");
    });
    ...
    function updateForm(ele) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                $('#updateModal [data-my="disAvatar"]').attr('src',result.dataZone.obj.gimg== '' ? '/upload/null.png' : result.dataZone.obj.gimg);
                $('#updateModal [name="gintroduction"]').val(result.dataZone.obj.gintroduction);
                $('#updateModal [name="gprice"]').val(result.dataZone.obj.gprice);
                $('#updateModal [name="gbid"]').val(result.dataZone.obj.gbid);
                choice1 = result.dataZone.obj.gbid;
                $('#updateModal [name="gcgid"]').val(result.dataZone.obj.gcgid);
                choice2 = result.dataZone.obj.gcgid;
        ...
        //填充列表
        getAndFill("${app}/business/listJSON",$("#uglist"),"bid","bname",choice1);
        getAndFill("${app}/categorys/listJSON",$("#ugclist"),"cgid","cgname",choice2);
    ...
    function parseDataAndShow(result) {
        ...
        $.each(lists, function (index, item) {
            ...
            var td5 = $('<td></td>').text(item.gimg);
            var td3 = $('<td></td>').text(item.gintroduction);
            var td4 = $('<td></td>').text(item.gprice.toFixed(2));
            var td6 = $('<td></td>').text(item.business.bname);
            var td7 = $('<td></td>').text(item.categorys.cgname);
            var addTimeTd = $('<td></td>').text(new Date(item.addTime).Format("yyyy-MM-dd HH:mm:ss"));
            var upBtnTd = $('<td></td>').html('<a class="upBtn btn btn-info btn-sm" href="${app}/goods/opt/' + item.gid + '">修改</a>');
            var delBtnTd = $('<td></td>').html('<a class="delBtn btn btn-danger btn-sm" href="${app}/goods/opt/' + item.gid + '">删除</a>');
            //将单元格追加到行中
